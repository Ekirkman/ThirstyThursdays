...
<div class="row">
  ...
  <c:set value="${category}" var="cat"/>
  <c:choose>
  <c:when test="${cat == 'forSale'}">
    <h1>For Sale</h1>
  </c:when>
  <c:when test="${cat eq 'jobs'}">
    <h1>Jobs</h1>
  </c:when>
  <c:when test="${cat eq 'housing'}">
    <h1>Housing</h1>
  </c:when>
  <c:when test="${cat eq 'connections'}">
    <h1>Connections</h1>
  </c:when>
  <c:when test="${cat eq 'services'}">
    <h1>Services</h1>
  </c:when>
  <c:when test="${cat eq 'events'}">
    <h1>Events</h1>
  </c:when>
  <c:when test="${cat eq 'LostnFound'}">
    <h1>Lost & Found</h1>
  </c:when>
</c:choose>
  ...
  <div class="form-group col-md-6">
 				 ...
 				 <input type="button" name="Search" id="search" class="btn btn-primary btn-sm" value="search" onclick="location.href='./searchPost?mySearch='+ document.getElementById('search').value;">
 				
              </div>
 			</div>
 			<br>

		<c:forEach items="${categoryPost}" var="post">
			<div class="col-xs-12 col-md-4 section-container-spacer">
				<img alt="" src="data:image/jpg;base64,${post.showPicture()}" width="300px" height="200px">
				<h2>${post.postTitle}</h2>
				<p>${post.postDescription}</p>
				<a href="./postDetails?postId=${post.postId}" class="btn btn-primary" title="">
					Contact Poster</a>
			</div>
